...
#include "gfx_legacy/ui_renderer.h"
#include "base/util.h"
...
public:
    TimerHandle AddTimer(Object* object,
                         const TimerCallback& callback,
                         uint64_t periodMs) {
        timers_.emplace_back(
            Timer(object->GetWeak(), callback, periodMs, Now()));
...
private:
    constexpr uint64_t DurationMs(const TimePoint& p0, const TimePoint& p1) {
        return p1 >= p0 ? std::chrono::duration_cast<std::chrono::milliseconds>(
                              p1 - p0)
                              .count()
...
public:
    VerticalTextDrawer(DrawList* drawList, Point cursor, Color color)
        : drawList_(drawList), cursor_(cursor), color_(color), indent_(0) {}
    ...
    void DrawText(std::string_view text) {
        drawList_->DrawText(cursor_ + float2(indent_, 0.f), color_,
                            text.data());
        cursor_ += cursorOffset;
    }
    ...
    void DrawTextF(const std::format_string<ArgTypes...> format,
                   ArgTypes... args) {
        drawList_->DrawText(
            cursor_ + float2(indent_, 0.f), color_,
...
public:
    CanvasImpl(DrawList* drawList) : drawList(drawList) {}
    ...
    void DrawBox(Rect rect, const BoxStyle* style) override {
        ...
        if (style->borderAsOutline) {
            ...
            backgroundRect = Rect(rect).Expand(
                (float)-style->borders.Top, (float)-style->borders.Right,
        ...
        if (style->borders.Left || style->borders.Right || style->borders.Top ||
            style->borders.Bottom) {
            if (backgroundColor.a != 1.f) {
                drawList->DrawRect(borderRect, ColorU32(borderColor),
                                   style->rounding,
                                   (DrawList::Corner)style->roundingMask);
            } else {
                drawList->DrawRectFilled(borderRect, ColorU32(borderColor),
                                         style->rounding,
                                         (DrawList::Corner)style->roundingMask);
            }
        }

        drawList->DrawRectFilled(backgroundRect, ColorU32(backgroundColor),
                                 style->rounding,
                                 (DrawList::Corner)style->roundingMask);
    }
    ...
    void DrawText(Point origin,
                  ...
                  std::string_view textView) override {
        origin = Transform(origin);
        drawList->DrawText(origin, style->color, textView, style->fontSize,
                           style->fontWeightBold, style->fontStyleItalic);
    }
    ...
    void PushTransform(float2 transform) { context.transform = transform; }
    ...
    void ClearContext() {
        ...
        DASSERT(float2(std::round(cummulativeTransform.x),
                       std::round(cummulativeTransform.y)) == float2());
    }

    bool HasClipRect() const { return context.hasClipRect; }

    Point Transform(Point point) { return point + cummulativeTransform; }
...
struct AncestorIterator {
    constexpr AncestorIterator(Widget* w, bool includeBegin = false)
        : begin_(w), includeBegin_(includeBegin) {}
...
public:
    ...
    struct Iterator {
        ...
        Widget* operator*() { return it->Get(); }

        iterator it;
        iterator se;
    };

    WidgetList() { cont.reserve(10); }

    void Push(Widget* item) { cont.push_back(item->GetWeak()); }
    ...
    Widget* Top() { return cont.empty() ? nullptr : cont.front().GetChecked(); }

    const Widget* Top() const {
        return cont.empty() ? nullptr : cont.front().GetChecked();
    }
    ...
    constexpr Iterator end() { return {cont.end(), cont.end()}; }
 ...
 */
class ApplicationImpl final : public ui::Application, public Widget {
public:
    ...
    void Init() {
        ...
        nativeWindow->SetOnCursorMoveCallback([](float x, float y) {
            application->DispatchMouseMoveEvent({x, y});
        });
        nativeWindow->SetOnMouseButtonCallback(
            [](KeyCode button, bool bPressed) {
                application->DispatchMouseButtonEvent(button, bPressed);
            });
        nativeWindow->SetOnMouseScrollCallback([](float scroll) {
            application->DispatchMouseScrollEvent(scroll);
        });
        nativeWindow->SetOnWindowResizedCallback([](float2 windowSize) {
            application->DispatchOSWindowResizeEvent(windowSize);
        });
        nativeWindow->SetOnKeyboardButtonCallback(
            [](KeyCode button, bool bPressed) {
                application->DispatchKeyEvent(button, bPressed);
            });
        nativeWindow->SetOnCharInputCallback([](wchar_t character) {
            application->DispatchCharInputEvent(character);
        });
    ...
    void DispatchMouseMoveEvent(Point mousePosGlobal) {
        ...
        mouseMoveEvent.mouseButtonsPressedBitField =
            mouseButtonsPressedBitField_;
        ...
        // If we're update the hittest after rebuild, delta will be 0.0 so
        // ingore mouse drags
        ...
        if (!capturingWidgets_.Empty()) {
            ...
            for (auto* w : capturingWidgets_) {
                const auto mouseDeltaFromInitial =
                    mousePosGlobal - mousePosOnCaptureGlobal_;
                // Convert global to local using hit test data
                const auto* parentHitData =
                    lastHitStack_.Find(w->FindAncestorOfClass<LayoutWidget>());
                const auto mousePosLocal =
                    parentHitData
                        ? parentHitData->hitPosLocal + mouseDeltaFromInitial
                        : mousePosGlobal;

                MouseDragEvent dragEvent;
                dragEvent.mousePosOnCaptureLocal =
                    mousePosLocal - mouseDeltaFromInitial;
        ...
        // Dispatch leave events first so that event timeline is consistent for
        // widgets I.e. if some widget receives a hover enter event it will
        // reveive a hover leave before it's neigboor receives enter
        ...
        if (!hitStack.Empty()) {
            ...
            for (auto* widget : hoveredWidgets) {
                ...
                const bool isAlways =
                    isMouseRegion &&
                    widget->As<MouseRegion>()->ShouldAlwaysReceiveHover();
    ...
    void DispatchMouseButtonEvent(KeyCode button, bool bPressed) {
        ...
        if (bPressed && mouseButtonHeldNum_ == 1) {
            ...
            for (auto it = lastHitStack_.begin(); it != lastHitStack_.end();
                 ++it) {
                ...
                do {
                    // Mouse regions have an option to always receive events, so
                    // they cannot block them from propagating
        ...
        if (!capturingWidgets_.Empty() && !bPressed &&
            (MouseButton)button == pressedMouseButton_) {
            MouseButtonEvent e;
    ...
    void DispatchKeyEvent(KeyCode button, bool bPressed) {
        if (button == KeyCode::KEY_LEFT_SHIFT)
            modifiersState_[LeftShift] = !modifiersState_[LeftShift];
        if (button == KeyCode::KEY_RIGHT_SHIFT)
            modifiersState_[RightShift] = !modifiersState_[RightShift];

        if (button == KeyCode::KEY_LEFT_CONTROL)
            modifiersState_[LeftControl] = !modifiersState_[LeftControl];
        if (button == KeyCode::KEY_RIGHT_CONTROL)
            modifiersState_[RightControl] = !modifiersState_[RightControl];

        if (button == KeyCode::KEY_LEFT_ALT)
            modifiersState_[LeftAlt] = !modifiersState_[LeftAlt];
        if (button == KeyCode::KEY_RIGHT_ALT)
            modifiersState_[RightAlt] = !modifiersState_[RightAlt];

        if (button == KeyCode::KEY_CAPS_LOCK)
            modifiersState_[CapsLock] = !modifiersState_[CapsLock];
    ...
    void DispatchMouseScrollEvent(float scroll) {
        ...
        for (auto* mouseRegion :
             AncestorIterator<MouseRegion>(lastHitStack_.TopWidget(), true)) {
    ...
    std::string PrintContext() {
        std::string out;
        auto sb = StringBuilder(&out);
        sb.Line("Application context:\n");

        sb.Line("Opened window count: {}", widgetStack_.size());
        sb.Line("Timers count: {}", timers_.Size());
        sb.Line("Hovered window: {}",
                hoveredWindow_ ? hoveredWindow_->GetDebugID() : "");
        sb.Line("Hovered widget: {}",
                !hoveredWidgets_.Empty() ? hoveredWidgets_.Print() : "");
        sb.Line("Mouse pos local: {}",
                hoveredWidgets_.Top()
                    ? GetLocalPosForWidget(hoveredWidgets_.Top())
                    : mousePosGlobal_);
        sb.Line("Mouse pos global: {}",
                mousePosGlobal_ == Point::Max()
                    ? "OUTSIDE"
                    : std::format("{}", mousePosGlobal_));
        sb.Line("Capturing mouse widgets: {}",
                !capturingWidgets_.Empty() ? capturingWidgets_.Print() : "");
        sb.Line();
        ...
        for (auto it = widgetStack_.rbegin(); it != widgetStack_.rend(); ++it) {
            ...
            auto visitor = [&](PropertyArchive::Object& object) -> bool {
                const auto parentID =
                    object.parent_ ? object.parent_->objectID_ : 0;
                ...
                if (!parentID) {
                    ...
                    for (auto stackParentID = parentIDStack.back();
                         stackParentID != parentID; parentIDStack.pop_back(),
                              stackParentID = parentIDStack.back())
                        ;
                }
    ...
    std::string PrintAncestors(Widget* widget) {
        ...
        for (auto* w : AncestorIterator(widget, true)) {
            ar.PushObject(w->GetDebugID(), w, (Widget*)nullptr);
            w->DebugSerialize(ar);
        }
        StringBuilder sb(&out);
    ...
    std::string PrintPropertyArchive(const PropertyArchive& ar,
        ...
        if (reversed) {
            for (auto it = ar.rootObjects_.begin(); it != ar.rootObjects_.end();
                 ++it) {
                print(**it);
            }
        } else {
            for (auto it = ar.rootObjects_.rbegin();
                 it != ar.rootObjects_.rend(); ++it) {
                print(**it);
            }
        }
        return out;
    ...
    std::string PrintHitStack(uint32_t indent = 1) {
        ...
        for (auto& hitData : lastHitStack_) {
            if (!hitData.widget)
                continue;
            ar.PushObject(hitData.widget->GetDebugID(),
                          (Widget*)*hitData.widget, (Widget*)nullptr);
        ...
        for (auto& font : fonts) {
            if (font->NeedsRebuild()) {
                ...
                TextureHandle fontTexture =
                    renderer->CreateTexture(fontImageAtlas);
        ...
        drawFunc = [&](Widget* widget) {
            if (auto* layout = widget->As<LayoutWidget>()) {
                ...
                DASSERT_F(!rect.Empty(),
                          ...
                          "Widget: {}\nAncestors:\n{}",
                          layout->GetDebugID(), PrintAncestors(layout));
    ...
    void UpdateLayout(Widget* widget) {
        ...
        for (Widget* ancestor : AncestorIterator(widget)) {
            if (auto* l = ancestor->As<LayoutWidget>()) {
                ...
                const auto layoutDependsOnChildren =
                    axisMode.x == AxisMode::Shrink ||
                    axisMode.y == AxisMode::Shrink;
        ...
        if (isRoot) {
            ...
            e.rect = Rect(layoutWidget->GetOrigin() - layoutInfo.margins.TL(),
    ...
    // If the widget in the new tree has the same class as position as the
    // widget in the old tree 		the old widget is updated with the new one and all
    // raw pointers and weak pointers to it remain valid
        ...
        build = [&](StatefulWidget* widget) {
            ...
            if (!state) {
                ...
                DASSERT_F(state != nullptr,
                          "Stafeful widget without state found during Build. "
                          "Ancestor tree:\n{}\nContext:\n{}",
                          s, PrintContext());
            }
            ...
            if (!widget->NeedsRebuild()) {
                if (!oldWidget || !oldWidget->GetChild()) {
                    ...
                    DASSERT_F(oldWidget != nullptr,
                              "A widget with clear state and no old children "
                              "is found while building the widget {}. Ancestor "
                              "tree:\n{}\nContext:\n{}",
                              requestedWidget->GetDebugID(), ancestors,
                              PrintContext());
                }
            ...
            // 		Update: The old widget is updated with the new one and stays
            // in the tree 		Swap: The old widget is swapped with the new one
            //		Discard: If widget types are different, whole old tree is
            ...
            diff = [&](Widget* oldWidget, Widget* newWidget, Widget* parent) {
                ...
                if (!newWidget) {
                    std::unique_ptr<Widget> discardWidget =
                        parent->Orphan(oldWidget);
                    return;
                }
                ...
                if (!oldWidget) {
                    if (newWidget->GetParent() != parent) {
                        parent->Parent(
                            newWidget->GetParent()->Orphan(newWidget));
                    }
                    newWidget->VisitChildrenRecursively(
                        [&build](Widget* child) {
                ...
                // If we continue diffing children this will own the widget for
                // diff and will be deleted on the way back
                std::unique_ptr<Widget> pendingDelete;

                const bool discard =
                    oldWidget->GetClass() != newWidget->GetClass();
                if (discard) {
                    if (newWidget->GetParent() != parent) {
                        std::unique_ptr<Widget> discardWidget =
                            parent->Orphan(oldWidget);
                    ...
                    if (oldWidget->GetParent() != parent) {
                        pendingDelete = std::move(
                            newWidget->GetParent()->Orphan(newWidget));
                        parent->Parent(
                            oldWidget->GetParent()->Orphan(oldWidget));
                    }
                } else {
                    newParent = newWidget;
                    if (newWidget->GetParent() != parent) {
                ...
                for (size_t index = 0; index < newWidgets.size(); ++index) {
                    auto* old =
                        index < oldWidgets.size() ? oldWidgets[index] : nullptr;
                    diff(old, newWidgets[index], newParent);
                }
                if (parent == oldWidget->GetParent() &&
                    oldWidgets.size() > newWidgets.size()) {
                    auto oldWidgetsToDeleteNum =
                        oldWidgets.size() - newWidgets.size();

                    for (auto i = oldWidgetsToDeleteNum; i < oldWidgets.size();
                         ++i) {
                        std::unique_ptr<Widget> toDeleteWidget =
                            parent->Orphan(oldWidgets[i]);
                    }
                }
            };
...
public:
    ...
    bool Tick() {
        const auto frameStartTimePoint =
        ...
        const auto frameEndTimePoint =
            std::chrono::high_resolution_clock::now();
        ++frameNum_;
        lastFrameTimeMs_ =
            std::chrono::duration_cast<std::chrono::microseconds>(
                frameEndTimePoint - frameStartTimePoint)
                .count() /
            1000.f;
    ...
    void RequestRebuild(StatefulWidget* widget) override {
        auto it = std::ranges::find_if(dirtyWidgets_, [&](const auto& w) {
    ...
    void BringToFront(Widget* widget) override {
        ...
        for (auto it = widgetStack_.begin(); it != widgetStack_.end(); ++it) {
            ...
            if (found) {
                ...
                if (nextItem != widgetStack_.end() &&
                    nextItem->layer != Layer::Overlay) {
    ...
    void Parent(std::unique_ptr<Widget>&& widget, Layer layer) override {
        ...
        if (layer == Layer::Overlay) {
            ...
            for (auto it = widgetStack_.begin(); it != widgetStack_.end();
                 ++it) {
                // Insert the window before overlays
                if (it->layer == Layer::Overlay) {
                    widgetStack_.insert(it,
                                        RootWidget(std::move(widget), layer));
                    break;
                }
            }
        }
    ...
    void RequestFocus(FocusNode* node) {
        ...
        for (auto* focusedAncestor :
             AncestorIterator<Focused>(node->GetWidget(), true)) {
            newChain.Push(focusedAncestor);
        }
        LOGF(Verbose, "Focus request is handled.\nOld chain:{}\nNew chain:{}",
             oldChain.Print(), newChain.Print());
    ...
    bool OnEvent(IEvent* e) { return false; }
...
private:
    ...
    std::unique_ptr<DebugOverlayWindow> debugOverlay_;

    bool bDrawDebugInfo_ = true;
    bool bDrawDebugLayout_ = false;
    bool bDrawDebugClipRects_ = false;

    // Stack of windows, bottom are background windows and top are overlay
    // windows Other windows in the middle
    struct RootWidget {
...
ui::Application* ui::Application::Create(std::string_view windowTitle,
                                         uint32_t width,
                                         uint32_t height) {
    if (!nativeWindow) {
        nativeWindow = INativeWindow::createWindow(windowTitle, width, height);
    }
    if (!application)
