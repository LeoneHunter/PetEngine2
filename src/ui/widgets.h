...
using PopupBuilderFunc =
    std::function<std::vector<std::unique_ptr<WidgetState>>(
...
public:
    ...
    static std::unique_ptr<TextBox> New(const std::string& text,
    ...
    void DebugSerialize(PropertyArchive& archive) override {
        LayoutWidget::DebugSerialize(archive);
