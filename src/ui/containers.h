...
public:
    ...
    bool OnEvent(IEvent* event) override {
        if (auto* drawEvent = event->As<DrawEvent>()) {
            drawEvent->canvas->DrawBox(GetRect(),
                                       style_->Find<BoxStyle>(boxStyleName_));
            if (bClip_) {
    ...
    void DebugSerialize(PropertyArchive& archive) override {
        SingleChildLayoutWidget::DebugSerialize(archive);
        archive.PushProperty("Clip", bClip_);
        archive.PushStringProperty("StyleClass",
                                   style_ ? *style_->name_ : "null");
    ...
    void SetBoxStyleName(StringID name) { boxStyleName_ = name; }

protected:
    Container(StringID styleName, bool notifyOnLayout = false)
        : SingleChildLayoutWidget(styleName,
                                  SizeMode::Shrink(),
                                  notifyOnLayout) {}
...
public:
    ...
    auto& SizeFixed(float2 inSize) {
        bSizeFixed = true;
        axisMode = SizeMode::Fixed();
        size = inSize;
        return *this;
    }
    auto& SizeFixed(uint32_t x, uint32_t y) {
        bSizeFixed = true;
        axisMode = SizeMode::Fixed();
        size = float2((float)x, (float)y);
        return *this;
    }
    auto& SizeMode(SizeMode inMode) {
        axisMode = inMode;
        return *this;
    }
    auto& SizeMode(AxisMode x, AxisMode y) {
        axisMode = ui::SizeMode(x, y);
        return *this;
    }
    auto& Size(float2 inSize) {
        size = inSize;
        return *this;
    }
    auto& Size(float x, float y) {
        size = float2(x, y);
        return *this;
    }
    ...
    auto& NotifyOnLayoutUpdate() {
        bNotifyOnLayout = true;
        return *this;
    }
    auto& BoxStyle(StringID inStyleName) {
        boxStyleName = inStyleName;
        return *this;
    }
    auto& StyleClass(StringID inStyleName) {
        styleClass = inStyleName;
        return *this;
    }
    auto& ClipContent(bool bClip = true) {
        bClipContent = bClip;
        return *this;
    }
    auto& Child(std::unique_ptr<Widget>&& inChild) {
    ...
    std::unique_ptr<Container> New() {
        std::unique_ptr<Container> out(
            new Container(styleClass, bNotifyOnLayout));
        out->SetID(id);
        out->SetSize(size);
...
inline ContainerBuilder Container::Build() {
    return {};
}
...
public:
    ...
    Flexible(float flexFactor, std::unique_ptr<Widget>&& widget)
        : SingleChildWidget(""), flexFactor_(flexFactor) {
...
enum class ContentDirection { Column, Row };
...
enum class OverflowPolicy {
    ...
    ShrinkWrap,  // Do not decrease containter size below content size
                 // Could be used with Scrolled widget to scroll content on
                 // overflow
};
...
DEFINE_ENUM_TOSTRING_5(JustifyContent,
                       Start,
                       End,
                       Center,
                       SpaceBetween,
...
class FlexboxBuilder {
public:
    FlexboxBuilder& ID(const std::string& inID) {
        id = inID;
        return *this;
    }
    FlexboxBuilder& Style(const std::string& inStyle) {
        style = inStyle;
        return *this;
    }
    // Direction of the main axis
    FlexboxBuilder& Direction(Axis axis) {
        direction =
            axis == Axis::X ? ContentDirection::Row : ContentDirection::Column;
        return *this;
    }
    ...
    FlexboxBuilder& JustifyContent(ui::JustifyContent inJustify) {
        justifyContent = inJustify;
        return *this;
    }
    // Alignment on the cross axis
    FlexboxBuilder& Alignment(ui::Alignment inAlignment) {
        alignment = inAlignment;
        return *this;
    }
    FlexboxBuilder& AlignCenter() {
        alignment = Alignment::Center;
        return *this;
    }
    FlexboxBuilder& AlignStart() {
        alignment = Alignment::Start;
        return *this;
    }
    FlexboxBuilder& AlignEnd() {
    ...
    FlexboxBuilder& ExpandMainAxis(bool bExpand = true) {
        expandMainAxis = bExpand;
        return *this;
    }
    FlexboxBuilder& ExpandCrossAxis(bool bExpand = true) {
        expandCrossAxis = bExpand;
        return *this;
    }
    ...
    FlexboxBuilder& Children(
        std::vector<std::unique_ptr<Widget>>&& inChildren) {
...
inline FlexboxBuilder Flexbox::Build() {
    return {};
}
...
public:
    static auto New(Alignment horizontal,
                    Alignment vertical,
                    std::unique_ptr<Widget>&& child) {
        return std::make_unique<Aligned>(horizontal, vertical,
                                         std::move(child));
    }
    ...
    Aligned(Alignment horizontal,
        ...
        : SingleChildLayoutWidget("", SizeMode::Fixed())
    ...
    void DebugSerialize(PropertyArchive& archive) override {
        SingleChildLayoutWidget::DebugSerialize(archive);
        archive.PushProperty("AlignmentHorizontal",
                             horizontal_ == Alignment::Start    ? "Start"
                             : horizontal_ == Alignment::Center ? "Center"
                                                                : "End");
        archive.PushProperty("AlignmentVertical",
                             vertical_ == Alignment::Start    ? "Start"
                             : vertical_ == Alignment::Center ? "Center"
                                                              : "End");
    }
    ...
    void Align(LayoutWidget* child) {
        ...
        for (auto axis : axes2D) {
            ...
            if (alignment == Alignment::Center) {
                childPos[axis] =
                    (innerSize[axis] - childOuterSize[axis]) * 0.5f;
