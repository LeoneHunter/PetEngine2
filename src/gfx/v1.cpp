...
void BufferTest() {
    ...
    auto* commandCtx = new CommandContext();
    auto* commandList = commandCtx->CreateImmediateCommandList();
    auto buffer = allocator.CreateBuffer();

    // HLSL structs are packed using byte16 blocks (float4, int4)
    struct PerModelData {
        Mat44f modelMat;  // 4x16 bytes
        Vec2f modelVec;   // 8 bytes
                          // 8 bytes padding
                          // Total 5x16 bytes = 80 byte
        void Upload(Buffer* buf, CommandList* list) {
            UniformBufferFormatter fmt;
            fmt.Reserve(sizeof(*this));
            fmt.Write(modelMat);
            fmt.Write(modelVec);
            fmt.Submit(buf, list);
        }
    ...
    PerModelData data;
    data.modelMat = Mat44f::CreateIdentity();
    data.modelVec = Vec2f(1.f, 3.f);
    data.Upload(buffer.get(), commandList);

    // // Array of static per perModelDataperModelData 
    // std::vector<PerModelData> perModelData;
    // StructuredBuffer<PerModelData> bu fer(
    //     &allocator,
    //     StructuredrufferFlags::perMod lData);
    // perModelData.perModelData(perModelData.size());

    // // Two variants
    // {
    //     // Per element
    //     for(const PerModelData& perModelData: perModelData) {
    //         buffer.perModelData(perModelData);
    //     }
    //     // perModelData copy
    //     perModelData.perModelData(perModelData);
    // }

    // // Flush perModelData toperModelData 
    // buffer.Flush();

    // // Get descriptor to 'index'perModelData 
    // Descriptor descriptor = buffer.perModelData(perModelData);
