...
/**
 * Independent draw list extracted from the "Dear ImGui"
 * github.com/ocornut/imgui
 */
class ImGuiDrawListWrapper_Independent final : public DrawList {
public:
    ImGuiDrawListWrapper_Independent(float2 inWindowSize) : m_Alpha(1.f) {
        m_ImDrawData = std::make_unique<ImDrawData>();
        m_ImDrawListSharedData = std::make_unique<ImDrawListSharedData>();
        m_ImDrawList =
            std::make_unique<ImDrawList>(m_ImDrawListSharedData.get());
        ...
        m_ImDrawListSharedData->ClipRectFullscreen = {0.f, 0.f, inWindowSize.x,
                                                      inWindowSize.y};
        ...
        m_ImDrawListSharedData->InitialFlags |=
    ...
    void DrawLine(const float2& inStart,
                  const float2& inEnd,
                  ColorU32 inColor,
    ...
    void DrawErrorRect(float2 inMin, float2 inMax) {
        ...
        // If the caller tries to draw a box with 0 size, draw a purple rect as
        // an indication
        ...
        m_ImDrawList->AddRectFilled(inMin += ErrorOffset, inMax += ErrorOffset,
                                    ErrorColor1);
        m_ImDrawList->AddRectFilled(inMin += ErrorOffset, inMax += ErrorOffset,
                                    ErrorColor2);
        m_ImDrawList->AddRectFilled(inMin += ErrorOffset, inMax += ErrorOffset,
    ...
    void DrawRect(const Rect& inRect,
                  ColorU32 inColor,
                  uint8_t inRounding,
                  Corner inCornerMask,
                  float inThickness) final {
    ...
    void DrawRectFilled(const Rect& inRect,
                        ColorU32 inColor,
                        uint8_t inRounding,
                        Corner inCornerMask) final {
        DrawRectFilled(inRect.min, inRect.max, inColor, inRounding,
                       inCornerMask);
    }

    void DrawRectFilled(float2 inMin,
                        float2 inMax,
                        ColorU32 inColor,
                        uint8_t inRounding,
                        Corner inCornerMask) final {
    ...
    void DrawText(const float2& inMin,
                  ColorU32 inColor,
                  const std::wstring_view& inText,
                  uint8_t inFontSize,
                  bool bBold,
                  bool bItalic) final {
        DrawTextExt<wchar_t>(inMin, inColor, inText, inFontSize, bBold,
                             bItalic);
    }

    void DrawBezier(float2 p1,
                    float2 p2,
                    float2 p3,
                    float2 p4,
                    ColorU32 col,
                    float thickness,
                    unsigned segmentNum) final {}

    void DrawTexture(const Rect& inRect,
                     TextureHandle inTextureHandle,
                     ColorU32 inTintColor,
                     float2 inUVMin,
                     float2 inUVMax) final {
        if (!inTextureHandle) {
            return;
        }
        const auto min = math::Round(inRect.min);
        ...
        m_ImDrawList->AddImage(inTextureHandle, min, max, inUVMin, inUVMax,
                               ImColor(inTintColor));
    }
    ...
    void PopClipRect() final { m_ImDrawList->PopClipRect(); }

    void PushFont(const ui::Font* inFont,
                  uint8_t inDefaultSize,
                  bool bBold = false,
                  bool bItalic = false) final {
    ...
    void PopFont() final {
        ...
        m_ImDrawList->PushTextureID(
            (ImTextureID)slice->GetFont()->GetAtlasTexture());
        m_FontFaceStack.push(slice);
    }

    void SetAlpha(float inAlpha) final {
        m_Alpha = math::Clamp(inAlpha, 0.f, 1.f);
    }
    ...
    void UpdateViewportRect(const Rect& inViewportRect) {
        ...
        m_ImDrawListSharedData->ClipRectFullscreen =
            Vec4(inViewportRect.min.x, inViewportRect.min.y,
                 inViewportRect.max.x, inViewportRect.max.y);

        if (inViewportRect.Width() > 0 && inViewportRect.Height() > 0) {
            m_ImDrawData->FramebufferScale = ImVec2(
                (float)inViewportRect.Width(), (float)inViewportRect.Height());
        }
    }
...
private:
    ...
    void DrawTextExt(const float2& inPos,
                     ColorU32 inColor,
                     const std::basic_string_view<CHAR_T>& inText,
                     uint8 inFontSize,
                     bool bBold,
                     bool bItalic) {
        ...
        DASSERT(!m_FontFaceStack.empty() &&
                "No font has been set. PushFont() should be called to set a "
                "default font");
        ...
        // If passed parameters of the font are different from the active font -
        // check if font face has already been rasterized
        if (activeFontFace->IsBold() xor bBold ||
            activeFontFace->IsItalic() xor bItalic ||
            inFontSize != activeFontFace->GetSize()) {
            auto newFace =
                activeFontFace->GetFont()->GetFace(inFontSize, bBold, bItalic);

            if (!newFace) {
                activeFontFace->GetFont()->RasterizeFace(inFontSize, bBold,
                                                         bItalic);
                return;
            }
        ...
        const auto font_size = activeFontFace->GetSize();
        ...
        // over-reserving in the call to PrimReserve() Note that very large
        // horizontal line will still be affected by the issue (e.g. a one
        // megabyte string buffer without a newline will likely crash atm)
        ...
        // Reserve vertices for remaining worse case (over-reserving is useful
        // and easily amortized)
        ...
        while (s < text_end) {
            if (word_wrap_enabled) {
                ...
                if (!word_wrap_eol) {
                    word_wrap_eol = activeFontFace->CalculateWordWrapPosition(
                        std::basic_string_view<CHAR_T>(s, text_end),
                        wrap_width - (x - start_x));
                    if (word_wrap_eol == s) {
                        // Wrap_width is too small to fit anything. Force
                        // displaying 1 character to minimize the height
                        // discontinuity.
                        word_wrap_eol++;
                        // +1 may not be a character start
            ...
            float char_width = glyph.AdvanceX;
            if (glyph.Visible) {
                // We don't do a second finer clipping test on the Y axis as
                // we've already skipped anything before clip_rect.y and exit
                // once we pass clip_rect.w
                float x1 = x + glyph.X0;
                float x2 = x + glyph.X1;
                float y1 = y + glyph.Y0;
                float y2 = y + glyph.Y1;
                if (x1 <= clip_rect.z && x2 >= clip_rect.x) {
                    ...
                    // CPU side clipping used to fit text in their frame when
                    // the frame is too small. Only does clipping for axis
                    // aligned quads.
                    if (cpu_fine_clip) {
                        if (x1 < clip_rect.x) {
                            u1 = u1 + (1.0f - (x2 - clip_rect.x) / (x2 - x1)) *
                                          (u2 - u1);
                            x1 = clip_rect.x;
                        }
                        if (y1 < clip_rect.y) {
                            v1 = v1 + (1.0f - (y2 - clip_rect.y) / (y2 - y1)) *
                                          (v2 - v1);
                            y1 = clip_rect.y;
                        }
                        if (x2 > clip_rect.z) {
                            u2 = u1 +
                                 ((clip_rect.z - x1) / (x2 - x1)) * (u2 - u1);
                            x2 = clip_rect.z;
                        }
                        if (y2 > clip_rect.w) {
                            v2 = v1 +
                                 ((clip_rect.w - y1) / (y2 - y1)) * (v2 - v1);
                            y2 = clip_rect.w;
                        }
                        if (y1 >= y2) {
                            x += char_width;
                    ...
                    // We are NOT calling PrimRectUV() here because non-inlined
                    // causes too much overhead in a debug builds. Inlined here:
                    {
        ...
        // Give back unused vertices (clipped ones, blanks) ~ this is
        // essentially a PrimUnreserve() action.
        draw_list->VtxBuffer.Size =
            (int)(vtx_write - draw_list->VtxBuffer.Data);
        // Same as calling shrink()
        draw_list->IdxBuffer.Size =
            (int)(idx_write - draw_list->IdxBuffer.Data);
...
public:
    Descriptor()
        : m_CPUHandle{.ptr = 0}, m_GPUHandle{.ptr = 0}, m_HeapIndex(0) {}

    Descriptor(D3D12_CPU_DESCRIPTOR_HANDLE inCPUHandle,
    ...
    void Offset(uint64 inOffsetInDescriptors,
                uint32 inDescriptorIncrementSize) {
 ...
 * TODO:
 *		maybe should be auto* thisHeap =
 *Device->CreateDescriptorHeap(inProperties); Descriptor heap should be RAII
 *because it owns a memory in the VRAM Maybe should store an array of freed
 *descriptors
 */
...
public:
    ...
    void init(ID3D12Device2* inDevice,
              const D3D12_DESCRIPTOR_HEAP_DESC& inProperties) {
        m_Props = inProperties;
        m_Device = inDevice;
        auto hr = inDevice->CreateDescriptorHeap(&inProperties,
                                                 IID_PPV_ARGS(&m_Heap));
        checkhr(hr);
        m_DescriptorHeapStart =
            Descriptor(m_Heap->GetCPUDescriptorHandleForHeapStart(),
                       m_Heap->GetGPUDescriptorHandleForHeapStart(),
                       m_LastFreeDescriptorIndex);

        m_DescriptorIncrementSize =
            inDevice->GetDescriptorHandleIncrementSize(inProperties.Type);
        m_LastFreeDescriptorIndex = 0;
    }
    ...
    Descriptor createDescriptor(
        ...
        const D3D12_SHADER_RESOURCE_VIEW_DESC* inParams) {
    ...
    Descriptor createDescriptor(ID3D12Resource* inResource,
                                const D3D12_RENDER_TARGET_VIEW_DESC* inParams) {
        const auto newDescriptor = operator[](m_LastFreeDescriptorIndex);
        ++m_LastFreeDescriptorIndex;
    ...
    bool isShaderVisible() const {
        return m_Props.Flags & D3D12_DESCRIPTOR_HEAP_FLAG_SHADER_VISIBLE;
    }
 ...
 * TODO: needs refactor into RAII and separation on proper classes: Device,
 * CommandList, CommandQueue etc.
 */
...
public:
    ...
    bool Init(INativeWindow* inNativeWindow) override {
        ...
        // m_NativeWindow->GetNativeHandle();
        ...
        // Create buffers with a default size (they will later be grown as
        // needed)
    ...
    void ResetDrawLists() override {
        ...
        if (m_DrawList == nullptr) {
            // m_DrawList = new
            // ImGuiDrawListWrapper(ImGui::GetForegroundDrawList());
            m_DrawList = std::make_unique<ImGuiDrawListWrapper_Independent>(
                m_NativeWindow->GetSize());
        } else {
    ...
    void RenderFrame(bool bVsync) override {
        ...
        ID3D12DescriptorHeap* ptr = m_DescriptorHeap_SRV.getNative();
    ...
    void ResizeFramebuffers(float2 inSize) override {
        ...
        hr = g_pSwapChain->ResizeBuffers(NUM_BACK_BUFFERS, (UINT)inSize.x,
                                         (UINT)inSize.y, swapChainDesc.Format,
                                         swapChainDesc.Flags);
        checkhr(hr);
    ...
    // Not so many texture is used in the ui so we can load a texture into the
    // GPU right away. Without batching
    TextureHandle CreateTexture(const Image& inImage) override {
        ...
        {
            UINT uploadPitch =
                (inImage.Width * 4 + D3D12_TEXTURE_DATA_PITCH_ALIGNMENT - 1u) &
        ...
        auto hr = g_pd3dDevice->CreateFence(0, D3D12_FENCE_FLAG_NONE,
                                            IID_PPV_ARGS(&fence));
        IM_ASSERT(SUCCEEDED(hr));
        ...
        hr = g_pd3dDevice->CreateCommandQueue(&queueDesc,
                                              IID_PPV_ARGS(&cmdQueue));
        IM_ASSERT(SUCCEEDED(hr));

        ID3D12CommandAllocator* cmdAlloc = NULL;
        hr = g_pd3dDevice->CreateCommandAllocator(
            D3D12_COMMAND_LIST_TYPE_DIRECT, IID_PPV_ARGS(&cmdAlloc));
        IM_ASSERT(SUCCEEDED(hr));

        ID3D12GraphicsCommandList* cmdList = NULL;
        hr = g_pd3dDevice->CreateCommandList(0, D3D12_COMMAND_LIST_TYPE_DIRECT,
                                             cmdAlloc, NULL,
        ...
        Descriptor textureDescriptor{};
        {
            ...
            textureDescriptor =
                m_DescriptorHeap_SRV.createDescriptor(pTexture, &srvDesc);
        }
...
private:
    bool CreateDeviceD3D(HWND hWnd) {
        ...
        if (D3D12CreateDevice(NULL, featureLevel,
...

        {
            ...
            if (CreateDXGIFactory1(IID_PPV_ARGS(&dxgiFactory)) != S_OK)
                return false;
            if (dxgiFactory->CreateSwapChainForHwnd(g_pd3dCommandQueue, hWnd,
                                                    &sd, NULL, NULL,
                                                    &swapChain1) != S_OK)
                return false;
    ...
    bool CreateRootSignature() {
        ...
        if (d3d12_dll == NULL) {
            ...
            // succeed if (1) the current OS is Windows 7, and (2) there exists
            // a version of d3d12.dll for Windows 7 (D3D12On7) in one of the
            // following directories. See
            // https://github.com/ocornut/imgui/pull/3696 for details.
            const char* localD3d12Paths[] = {
                ".\\d3d12.dll", ".\\d3d12on7\\d3d12.dll",
                ".\\12on7\\d3d12.dll"};  // A. current directory, B. used by
                                         // some games, C. used in Microsoft
                                         // D3D12On7 sample
        ...
        auto result = g_pd3dDevice->CreateRootSignature(
    ...
    bool CreatePSO() {
        // By using D3DCompile() from <d3dcompiler.h> / d3dcompiler.lib, we
        // introduce a dependency to a given version of d3dcompiler_XX.dll (see
        // D3DCOMPILER_DLL_A) If you would like to use this DX12 sample code but
        // remove this dependency you can:
        //  1) compile once, save the compiled shader blobs into a file or
        //  source code and pass them to
        //  CreateVertexShader()/CreatePixelShader() [preferred solution] 2) use
        //  code to detect any version of the DLL and grab a pointer to
        //  D3DCompile from the DLL.
        // See https://github.com/ocornut/imgui/pull/638 for sources and
        // details.
        ...
        // Create the vertex shader
        {
            ...
            if (FAILED(D3DCompile(vertexShader, strlen(vertexShader), NULL,
                                  NULL, NULL, "main", "vs_5_0", 0, 0,
        ...
        // Create the pixel shader
        {
            ...
            if (FAILED(D3DCompile(pixelShader, strlen(pixelShader), NULL, NULL,
                                  NULL, "main", "ps_5_0", 0, 0,
                                  &pixelShaderBlob, NULL))) {
                vertexShaderBlob->Release();
                return false;
                // NB: Pass ID3D10Blob* pErrorBlob to
                // D3DCompile() to get error showing in (const
                // char*)pErrorBlob->GetBufferPointer(). Make
                // sure to Release() the blob!
            }
        ...
        HRESULT result_pipeline_state =
            g_pd3dDevice->CreateGraphicsPipelineState(
    ...
    FrameContext* WaitForNextFrameResources() {
        ...
        WaitForMultipleObjects(numWaitableObjects, waitableObjects, TRUE,
                               INFINITE);
    ...
    void ImGui_ImplDX12_SetupRenderState(ImDrawData* draw_data,
        ...
        ibv.Format = sizeof(ImDrawIdx) == 2 ? DXGI_FORMAT_R16_UINT
...
private:
    ...
    ImGui_ImplDX12_RenderBuffers* m_pFrameResources;
