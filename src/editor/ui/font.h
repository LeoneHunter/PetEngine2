 ...
 *	- Font atlas which manages all variations of the same font. Variation are accessable through the Font::GetSlice().
 *	- Font atlas which manages all variations of the same font. Variation are accessable directly through the Font::GetParameters().
...
public:
	...
	public:
		...
		virtual float2 CalculateTextSize(const std::string_view& inText, float inMaxTextWidth, float inWrapWidth) const = 0;
		virtual float2 CalculateTextSize(const std::wstring_view& inText, float inMaxTextWidth, float inWrapWidth) const = 0;

		virtual const char* CalculateWordWrapPosition(const std::string_view& inText, float inWrapWidth) const = 0;
		virtual const wchar_t* CalculateWordWrapPosition(const std::wstring_view& inText, float inWrapWidth) const = 0;

		// Rendering parameters for the renderer
		virtual void GetRenderParameters(FaceRenderParameters* outParameters) const = 0;
