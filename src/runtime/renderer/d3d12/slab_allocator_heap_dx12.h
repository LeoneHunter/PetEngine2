...
constexpr size_t kMaxNumSlotsPerSpan = 64;
...
// 256 MiB
constexpr size_t kDefaultHeapSize = kPageSize * kNumPagesPerHeap;
...
// 16
constexpr size_t kMaxNumPagesPerSpan =
    (kMinSpanSize * kMaxSlotSize) / kPageSize;

constexpr size_t kGiB = 1024 * 1024 * 1024;
constexpr size_t kDefaultMaxNumSpansPerGiB =
...
public:
    ...
    public:
        ...
        void Commit(uint32_t slotSize) {
            ...
            const uint16_t numSlots =
                uint16_t((numPages_ * kPageSize) / slotSize);
    ...
    HeapMetadata(RefCountedPtr<ID3D12Heap> d3d12Heap,
        ...
        if (tracker_) {
            tracker_->DidCreateHeap(AllocationTracker::HeapInfo{
                .name = name, .address = (uintptr_t)this});
        }
    }
...
public:
    ...
    public:
        ...
        constexpr void Init(uint32_t numPages, uint32_t slotSize = 0) {
            ...
            if (numPages > 1) {
                const uint32_t lastPageIndex = GetPageIndex() + numPages - 1;
                DASSERT(lastPageIndex < GetNumPages());
                Span* lastPageMetadata = PageIndexToSpan(lastPageIndex);
                lastPageMetadata->InitAsLast(numPages);
            }
        }
...
public:
    HeapMetadata(RefCountedPtr<ID3D12Heap> d3d12Heap,
        ...
        , desc_(d3d12Heap->GetDesc())
        , heap_(std::move(d3d12Heap))
        , numPages_(uint32_t(d3d12Heap->GetDesc().SizeInBytes / internal::kPageSize))
...
public:
    constexpr FreeSpanCache(uint32_t maxNumPages)
        ...
        spanListHead_.resize(maxNumPages_);
    }
