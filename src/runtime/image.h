...
public:
    ...
    Image(size_t inWidth,
          size_t inHeight,
          Format inFormat,
          unsigned char* inData,
          size_t inDataSize = 0)
        : Width(inWidth), Height(inHeight), m_Format(inFormat), Data() {
        size_t dataSizeBytes = inDataSize;
        if (dataSizeBytes == 0) {
            dataSizeBytes =
                inWidth * inHeight * GetFormatBytesPerPixel(inFormat);
        }
    ...
    size_t GetFormatBytesPerPixel(Format inFormat) const {
        switch (inFormat) {
            ...
            case Image::Format::SRGB:
                return 3;
            case Image::Format::SRGBA:
                return 4;
            case Image::Format::LUMINANCE:
                return 1;
        }
        return 0;
    }
