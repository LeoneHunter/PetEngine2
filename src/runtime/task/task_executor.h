...
public:
    ...
    TaskExecutor(std::shared_ptr<TaskTracker> tracker) 
        : tracker_(tracker)
    {}
    ...
    // A client can hold the reference to the taskSource and continue to post tasks to it
    // Or the taskSource could be released and it will be deleted when all tasks have been processed
    // TODO: Destroy the task taskSource if has only one reference, or reuse for internal tasks
...
private:
    void WorkerMain(bool) override {
        executor_->RunUntilStopped(true);
    } 
