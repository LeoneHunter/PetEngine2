...
public:
    ...
    constexpr JsonObjectBuilder& Field(const std::string& name, Arg arg) {
        sb.Line("\"{}\": \"{}\",", name,
                std::format("{}", std::forward<Arg>(arg)));
        return *this;
    }
    ...
    template <Formattable... Args>
    constexpr JsonObjectBuilder& Array(const std::string& name,
                                       Args&&... args) {
        // arg1 -> [arg1]
        // arg1 arg2 -> [arg1, arg2]
        std::string fargs;
...
public:
    ...
    AllocTracker(bool printEvents) : printEvents(printEvents) {}
...
TEST_CASE("[d3d12::SlabAllocator_v2] Random Test") {
    ...
    auto result = DeviceFactory::CreateDevice(currentAdapter, kDebugBuild);
    DASSERT(result);
    RefCountedPtr<DeviceD3D12> device = *result;
    std::unique_ptr<HeapAllocator> heapAlloc =
        HeapAllocator::Create(device.Get(), D3D12_HEAP_TYPE_DEFAULT);
    std::unique_ptr<SlabAllocator> alloc =
        SlabAllocator::Create(heapAlloc.get(), tracker.get());
    // 1 GiB
    ...
    bench.Run(20);
    bench::Benchmark::Stats stats = bench.GetStats();
    ...
    Println("[d3d12::Allocator_v2 Random Test]   Num objects: {}", kNumObjects);
