...
DescriptorHeap::DescriptorHeap(DeviceD3D12* device,
                               D3D12_DESCRIPTOR_HEAP_DESC desc,
                               const std::string& debugName)
    : debugName_(debugName), desc_(desc) {
    DASSERT(device);
    auto hr = device->GetNative()->CreateDescriptorHeap(
        &desc_, IID_PPV_ARGS(&d3dHeap_));
    DASSERT_HR(hr);
...
std::optional<DescriptorLocation> DescriptorHeap::TryAllocate(size_t size) {
    ...
    for (size_t segmentIndex = 0; segmentIndex < segments_.size();
         ++segmentIndex) {
        ...
        // TODO: Deal with fragmentation or just ignore if modifications are
        // rare
...
void DescriptorHeap::Free(DescriptorLocation location) {
                ...
                DASSERT_F(
                    [&] {
                        const bool isLastSegment =
                            pivot + 1 >= segments_.size();
                        const size_t nextSegmentStartSlot =
                            isLastSegment ? GetCapacity()
                                          : segments_[pivot + 1].startSlotIndex;
                        return startSlotIndex >= nextSegmentStartSlot;
                    }(),
                    "Descriptor location points to the center of the allocated "
                    "segment");
                lowerBound = pivot;
            } else {
                higherBound = pivot;
    ...
    const bool mergeLeft =
        segmentIndex ? segments_[segmentIndex - 1].isFree : false;

    const size_t lastSegmentIndex = segments_.size() - 1;
    const bool mergeRight = segmentIndex < lastSegmentIndex
                                ? segments_[segmentIndex + 1].isFree
                                : false;
...
PooledDescriptorAllocator::PooledDescriptorAllocator(DeviceD3D12* device)
    : device_(device) {}
...
std::unique_ptr<PooledDescriptorAllocator> PooledDescriptorAllocator::Create(
    DeviceD3D12* parent,
    const std::string& debugName) {
    auto out = std::unique_ptr<PooledDescriptorAllocator>(
        new PooledDescriptorAllocator(parent));
...
DescriptorHeap& PooledDescriptorAllocator::CreateHeap(
    D3D12_DESCRIPTOR_HEAP_DESC desc,
    const std::string& debugName) {
    heaps_.emplace_back(new DescriptorHeap(device_, desc, debugName));
    return *heaps_.back();
}
...
DescriptorLocation PooledDescriptorAllocator::Allocate(
    ...
    for (auto& ptr : heaps_) {
        ...
        std::optional<DescriptorLocation> out =
            heap.TryAllocate(desc.NumDescriptors);
        if (out) {
            return *out;
        }
    }
    ...
    std::optional<DescriptorLocation> out =
        newHeap.TryAllocate(desc.NumDescriptors);
    DASSERT(out);
    return *out;
}
