...
public:
	...
	NativeWindow_Windows(const char* inWindowTitle, u32 inWidth, u32 inHeight) {
		...
		::RegisterClassExW(&m_WndCls);

		auto inWindowTitleWide = Util::ToWideString(std::string(inWindowTitle));
		m_hwnd = ::CreateWindowExW(0L, m_WndCls.lpszClassName, inWindowTitleWide.c_str(), WS_OVERLAPPEDWINDOW, windowPosX, windowPosY, windowWidth, windowHeight, NULL, NULL, m_WndCls.hInstance, NULL);
		...
		for(auto i = 1; i != (int)MouseCursor::_Size; ++i) {
			m_Cursors[i] = ::LoadCursorW(NULL, castCursor((MouseCursor)i));
		}
	}
	...
	void Destroy() override {
		...
		::UnregisterClassW(m_WndCls.lpszClassName, m_WndCls.hInstance);
