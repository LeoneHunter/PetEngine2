...
public:
	...
	constexpr			Vec2(const Vec2<OtherT>& in) : x(static_cast<T>(in.x)), y(static_cast<T>(in.y)) {}
...
template<Number T> constexpr Vec2<T> operator* (T left, const Vec2<T>& right) { return Vec2<T>(left * right.x, left * right.y); }
template<Number T> constexpr Vec2<T> operator* (const Vec2<T>& left, T right) { return Vec2<T>(left.x * right, left.y * right); }
template<Number T> constexpr Vec2<T> operator+ (float left, Vec2<T> right) { return Vec2<T>(left + right.x, left + right.y); }
template<Number T> constexpr Vec2<T> operator+ (const Vec2<T>& left, T right) { return Vec2<T>(left.x + right, left.y + right); }
template<Number T> constexpr Vec2<T> operator- (T left, const Vec2<T>& right) { return Vec2<T>(left - right.x, left - right.y); }
template<Number T> constexpr Vec2<T> operator- (const Vec2<T>& left, T right) { return Vec2<T>(left.x - right, left.y - right); }
template<Number T> constexpr Vec2<T> operator/ (const Vec2<T>& left, T right) { return Vec2<T>(left.x / right, left.y / right); }

template<Number T> constexpr Vec2<T> operator* (const Vec2<T>& left, const Vec2<T>& right) { return Vec2<T>(left.x * right.x, left.y * right.y); }
template<Number T> constexpr Vec2<T> operator/ (const Vec2<T>& left, const Vec2<T>& right) { return Vec2<T>(left.x / right.x, left.y / right.y); }
template<Number T> constexpr Vec2<T> operator- (const Vec2<T>& left, const Vec2<T>& right) { return Vec2<T>(left.x - right.x, left.y - right.y); }
template<Number T> constexpr Vec2<T> operator+ (const Vec2<T>& left, const Vec2<T>& right) { return Vec2<T>(left.x + right.x, left.y + right.y); }
template<Number T> constexpr Vec2<T>& operator+= (Vec2<T>& left, const Vec2<T>& right) { left.x += right.x; left.y += right.y; return left; }
template<Number T> constexpr Vec2<T>& operator-= (Vec2<T>& left, const Vec2<T>& right) { left.x -= right.x; left.y -= right.y; return left; }
template<Number T> constexpr Vec2<T>& operator-= (Vec2<T>& left, T right) { left.x -= right; left.y -= right; return left; }
template<Number T> constexpr Vec2<T>& operator+= (Vec2<T>& left, T right) { left.x += right; left.y += right; return left; }
...
public:
	...
	constexpr			Vec4(float inX, float inY, float inZ, float inW) : x(inX), y(inY), z(inZ), w(inW) {}
...
struct Rect {
	...
	constexpr point_type	Center() const { return {(min.x + max.x) * 0.5f, (min.y + max.y) * 0.5f}; }
	...
	constexpr value_type&	Side(int inSide) {
		switch(inSide) {
			case SideTop:	return min.y;
			case SideBottom:return max.y;
			case SideLeft:	return min.x;
			case SideRight:	return max.x;
		}
		return min.y;
	}
	...
	constexpr bool		Intersects(const Rect& otherRect) const {
		if(max[0] < otherRect.min[0] || min[0] > otherRect.max[0]) return false;
		if(max[1] < otherRect.min[1] || min[1] > otherRect.max[1]) return false;
		return true;
	}
	...
	constexpr Rect&		Expand(value_type top, value_type right, value_type bottom, value_type left) {
		min.x -= left;
		max.x += right;
		min.y -= top;
		max.y += bottom;
...
constexpr Rect operator+(const Rect& lhs, const float2& rhs) { return {lhs.min + rhs, lhs.max + rhs}; }
constexpr Rect operator-(const Rect& lhs, const float2& rhs) { return {lhs.min - rhs, lhs.max - rhs}; }

constexpr Rect& operator+=(Rect& lhs, const float2& rhs) { lhs.min += rhs; lhs.max += rhs; return lhs; }
constexpr Rect& operator-=(Rect& lhs, const float2& rhs) { lhs.min -= rhs; lhs.max -= rhs; return lhs; }

constexpr bool operator==(const Rect& lhs, const Rect& rhs) { return lhs.min == rhs.min && lhs.max == rhs.max; }
constexpr bool operator!=(const Rect& lhs, const Rect& rhs) { return lhs.min != rhs.min || lhs.max != rhs.max; }
...
constexpr Rect RectAlignHorizontalLeft(const Rect& target, const Rect& object, float margin = 0.f) {
	auto temp = object;
...
constexpr Rect RectAlignHorizontalRight(const Rect& target, const Rect& object, float margin = 0.f) {
	auto temp = object;
...
constexpr Rect RectAlignHorizontalCenter(const Rect& target, const Rect& object) {
	auto temp = object;
...
constexpr Rect RectAlignVerticalTop(const Rect& target, const Rect& object, float margin = 0.f) {
	auto temp = object;
...
constexpr Rect RectAlignVerticalBottom(const Rect& target, const Rect& object, float margin = 0.f) {
	auto temp = object;
...
struct Circle {
	...
	constexpr	Circle(const point_type& center, value_type radius) : center(center), radius(radius) {}
	constexpr	Circle(value_type p0, value_type p1, value_type radius) : center(p0, p1), radius(radius) {}
...
namespace Math {
	...
	template<Number T>
	constexpr T Clamp(T inVal, T inMin = std::numeric_limits<T>::min(), T inMax = std::numeric_limits<T>::max()) {
	...
	constexpr void closestPointSegment(float2 inSegmentP0, float2 inSegmentP1, float2 inPoint, float2& outClosest) {
