...
public:
	...
	float	GetSize() const override {
		return m_ImFont->FontSize;
	}

	float2	CalculateTextSize(const std::string_view& inText, float inMaxTextWidth, float inWrapWidth) const override {
		return m_ImFont->CalcTextSizeA(m_ImFont->FontSize, inMaxTextWidth, inWrapWidth, inText.data(), inText.data() + inText.size());
	}

	float2	CalculateTextSize(const std::wstring_view& inText, float inMaxTextWidth, float inWrapWidth) const override {
		...
		while(text_char_cursor < text_end) {
			if(word_wrap_enabled) {
				...
				if(!word_wrap_eol) {
					word_wrap_eol = CalculateWordWrapPosition(wstring_view(text_char_cursor, text_end), wrap_width - line_width);
					if(word_wrap_eol == text_char_cursor) // Wrap_width is too small to fit anything. Force displaying 1 character to minimize the height discontinuity.
				...
				if(text_char_cursor >= word_wrap_eol) {
					...
					while(text_char_cursor < text_end) {
						...
						if(ImCharIsBlankW(c)) { text_char_cursor++; } else if(c == '\n') { text_char_cursor++; break; } else { break; }
					}
					continue;
				}
			}
			...
			const float char_width = ((int)text_char < m_ImFont->IndexAdvanceX.Size ? m_ImFont->IndexAdvanceX.Data[text_char] : m_ImFont->FallbackAdvanceX) * scale;
	...
	const char* CalculateWordWrapPosition(const std::string_view& inText, float inWrapWidth) const override {
		return m_ImFont->CalcWordWrapPositionA(1.f, inText.data(), inText.data() + inText.size(), inWrapWidth);
	}

	// From ImGui::ImFont
	const wchar_t* CalculateWordWrapPosition(const std::wstring_view& inText, float wrap_width) const override {
...
bool operator==(const FontKey& left, const FontKey& right) { return left.key == right.key; }
bool operator!=(const FontKey& left, const FontKey& right) { return left.key != right.key; }
bool operator<(const FontKey& left, const FontKey& right) { return left.key < right.key; }
bool operator>(const FontKey& left, const FontKey& right) { return left.key > right.key; }
...
public:
	...
	ImGuiFont(const Path& inFilepath)
		...
		filepath = m_Filepath.parent_path() / (m_Filepath.stem().string() + nameSuffix);
	...
	bool RasterizeFace(u8 inSize, bool bBold = false, bool bItalic = false) override {
		...
		if(m_Filepath.empty()) {
			...
			// If style is not available - rasterize normal style of the same size
			if(!bStyleAvailable) {
			...
			imFont = m_FontAtlas.AddFontFromFileTTF(filepath.string().c_str(), inSize, &config);
		}
	...
	bool Build(Image* outImage) override {
		...
		for(auto [key, pendingFace] : m_PendingFaces) {
			m_Faces.emplace(key, new ImFontFace(pendingFace, this, key.bBold, key.bItalic));
		}
	...
	void* GetAtlasTexture() const override {
...
private:
	...
	enum class Flags {
		None = 1 << 0,
		BoldAvailable = 1 << 1,	// File of this font with the Bold style is available
		ItalicAvailable = 1 << 2,	// File of this font with the Italic style is available
		BoldItalicAvailable = 1 << 3	// File of this font with the Bold Italic style is available
	};
